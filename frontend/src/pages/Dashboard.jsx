...
export default function Dashboard() {
  ...
  if (!stats) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-gray-400 flex items-center gap-2"><Activity size={20} className="animate-pulse" /> Yükleniyor...</div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-400">{new Date().toLocaleDateString('tr-TR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Toplam Cihaz" value={stats.total_devices} sub={`${stats.active_devices} aktif, ${stats.scheduled_devices} zamanlanmış`} icon={Server} color="text-cyan-600" bg="bg-cyan-50" />
        <StatCard label="Toplam Yedek" value={stats.total_backups} sub={`Toplam boyut: ${formatSize(stats.total_backup_size)}`} icon={Database} color="text-emerald-600" bg="bg-emerald-50" />
        <StatCard label="Başarı Oranı" value={`%${stats.success_rate}`} sub={`${stats.successful_backups} başarılı / ${stats.failed_backups} başarısız`} icon={TrendingUp} color="text-green-600" bg="bg-green-50" />
        <StatCard label="Bugün" value={stats.today_backups} sub={stats.today_failed > 0 ? `${stats.today_failed} başarısız` : 'Tüm backup\'lar başarılı'} icon={Calendar} color={stats.today_failed > 0 ? 'text-red-600' : 'text-cyan-600'} bg={stats.today_failed > 0 ? 'bg-red-50' : 'bg-cyan-50'} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-4">Vendor Dağılımı</h2>
          {vendorData.length > 0 ? (
            <>
            ...
            <p className="text-gray-400 text-sm text-center py-8">Henüz cihaz eklenmemiş</p>
          )}
        </div>

        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-4">Lokasyon Dağılımı</h2>
          {locationData.length > 0 ? (
            ...
            <p className="text-gray-400 text-sm text-center py-8">Henüz lokasyon eklenmemiş</p>
          )}
        </div>

        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-4">Sistem Durumu</h2>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <span className="text-sm font-semibold text-gray-800">{stats.scheduled_devices} cihaz</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-2">
                <Server size={16} className="text-gray-500" />
                <span className="text-sm text-gray-600">Aktif Cihaz</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-4">Backup Trendi (Son 30 Gün)</h2>
          {trend.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={trend} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                ...
                <Tooltip labelFormatter={(d) => new Date(d).toLocaleDateString('tr-TR')} contentStyle={dark ? { backgroundColor: '#1f2937', border: '1px solid #374151', color: '#e5e7eb' } : undefined} />
                <Bar dataKey="success" name="Başarılı" fill="#22c55e" radius={[2, 2, 0, 0]} />
                <Bar dataKey="failed" name="Başarısız" fill="#ef4444" radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-4">Depolama Trendi (Son 30 Gün)</h2>
          {sizeTrend.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={sizeTrend} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                ...
                <Tooltip labelFormatter={(d) => new Date(d).toLocaleDateString('tr-TR')} formatter={(v) => [formatSize(v)]} contentStyle={dark ? { backgroundColor: '#1f2937', border: '1px solid #374151', color: '#e5e7eb' } : undefined} />
                <Area type="monotone" dataKey="cumulative" name="Toplam" stroke="#06b6d4" fill={dark ? '#164e63' : '#cffafe'} strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <p className="text-gray-400 text-sm text-center py-8">Henüz veri yok</p>
      ...
      <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-4">Son Aktiviteler</h2>
          ...
          <div className="space-y-2">
            {stats.recent_activities.map((a) => (
              <div key={a.id} className="flex items-center justify-between py-2.5 px-4 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-400">{a.triggered_by === 'manual' ? 'Manuel' : 'Zamanlanmış'}</span>
                  <span className="text-xs text-gray-500">{new Date(a.created_at).toLocaleString('tr-TR')}</span>
                </div>
              </div>
            ))}
          </div>
