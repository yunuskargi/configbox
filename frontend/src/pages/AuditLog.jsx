...
export default function AuditLog() {
  ...
  const actionLabels = {
    login: { label: t.audit_action_login, color: 'bg-cyan-100 text-cyan-700' },
    login_failed: { label: t.audit_action_login_failed, color: 'bg-red-100 text-red-700' },
    create: { label: t.audit_action_create, color: 'bg-green-100 text-green-700' },
    update: { label: t.audit_action_update, color: 'bg-yellow-100 text-yellow-700' },
    delete: { label: t.audit_action_delete, color: 'bg-red-100 text-red-700' },
    backup: { label: t.audit_action_backup, color: 'bg-purple-100 text-purple-700' },
  };

  const resourceLabels = {
    auth: t.audit_resource_auth,
    device: t.audit_resource_device,
    backup: t.audit_resource_backup,
    user: t.audit_resource_user,
    location: t.audit_resource_location,
    settings: t.audit_resource_settings,
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-800">{t.audit_title}</h1>
        </div>
      ...
      {showFilters && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex gap-4">
          ...
          <select value={filter.resource_type} onChange={(e) => setFilter((f) => ({ ...f, resource_type: e.target.value }))} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500">
            <option value="">{t.audit_all_resources}</option>
            {Object.entries(resourceLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
          <button onClick={() => setFilter({ action: '', resource_type: '' })} className="px-3 py-2 text-sm text-gray-500 hover:text-gray-700">{t.clear}</button>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 text-left">
            <tr>
              <th className="px-4 py-3 font-medium">{t.audit_time}</th>
              <th className="px-4 py-3 font-medium">{t.audit_user}</th>
              <th className="px-4 py-3 font-medium">{t.audit_action}</th>
              <th className="px-4 py-3 font-medium">{t.audit_resource}</th>
              <th className="px-4 py-3 font-medium">{t.audit_target}</th>
              <th className="px-4 py-3 font-medium">{t.detail}</th>
              <th className="px-4 py-3 font-medium">{t.audit_ip}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {logs.map((log) => {
              ...
              return (
                <tr key={log.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">{new Date(log.created_at).toLocaleString(locale)}</td>
            ...
            {logs.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">{t.audit_no_logs}</td></tr>
            )}
          </tbody>
        </table>
      </div>
